...
public class LoginActivity extends AppCompatActivity {
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            APIClient.getClient().login(new LoginRequest(email, password)).enqueue(new Callback<LoginResponse>() {
                @Override
                public void onResponse(Call<LoginResponse> call, Response<LoginResponse> response) {
                    showProgress(false);
                    SPrefUtils.setIntegerPreference(mAppContext, SPrefUtils.LOGIN_STATUS, KeyConstants.LOGIN_STATUS_LOGGED_IN);
                    Toast.makeText(mActivityContext, "Welcome " + response.body().getName() + "!", Toast.LENGTH_SHORT).show();
                    Log.i(TAG, "LoginResponse : " + response.body().getMessage());
